...
function testArtifactsFunctionality() {
    ...
    it("Should work with Contract.at", async function () {
        ...
        chai_1.assert.equal(await greeterWithAt.greet(), "Hi");
    ...
    it("Should work with new Contract(addr)", async function () {
        ...
        chai_1.assert.equal(await greeterWithNew.greet(), "Hi");
    ...
    it("Should deploy linked contracts succesfully", async function () {
        ...
        chai_1.assert.equal((await usesLib.n()).toString(), "0");
        await usesLib.addTwo();
        chai_1.assert.equal((await usesLib.n()).toString(), "2");
    });
}
...
describe("Test contracts compilation", function () {
    ...
    it("Should include sources from sources", async function () {
        const sources = await this.env.run(task_names_1.TASK_COMPILE_SOLIDITY_GET_SOURCE_PATHS);
...
describe("Gas multiplier", function () {
    async function assertItWorksForDeployments(env, multiplier) {
    ...
    async function assertItWorksForFunctions(env, multiplier) {
        const Greeter = env.artifacts.require("Greeter");
        const greeter = await Greeter.new();
    ...
    describe("When it's not set in the network", function () {
        (0, helpers_1.useEnvironment)("hardhat-project-solc-0.4");
        it("Should use the set one for deployments", async function () {
            await assertItWorksForDeployments(this.env, constants_1.DEFAULT_GAS_MULTIPLIER);
        });
        it("Should use the set one for functions", async function () {
            await assertItWorksForFunctions(this.env, constants_1.DEFAULT_GAS_MULTIPLIER);
        });
    });
});
