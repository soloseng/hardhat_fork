...
describe("compile task", function () {
    ...
    describe("project with single file", function () {
        ...
        it("should compile and emit artifacts", async function () {
            ...
            chai_1.assert.lengthOf((0, glob_1.globSync)("artifacts/build-info/*.json"), 1);
        });
    });
    describe("project with two files with different compiler versions", function () {
        (0, project_1.useFixtureProject)("compilation-two-files-different-versions");
        (0, environment_1.useEnvironment)();
        it("should compile and emit artifacts", async function () {
            await this.env.run("compile");
            assertFileExists(path.join("artifacts", "contracts", "A.sol", "A.json"));
            ...
            chai_1.assert.lengthOf((0, glob_1.globSync)("artifacts/build-info/*.json"), 2);
        });
    });
    describe("compilation jobs failure message", function () {
        ...
        it("should return a proper message for a non compatible solc error with a single file", async function () {
            ...
            chai_1.assert.equal(reasons, `The Solidity version pragma statement in these files doesn't match any of the configured compilers in your config. Change the pragma or configure additional compiler versions in your hardhat config.
    ...
    describe("old versions of solidity", function () {
        ...
        describe("project with an old version of solidity", function () {
            ...
            it("should throw an error", async function () {
                await (0, errors_1.expectHardhatErrorAsync)(async () => {
                    await this.env.run("compile");
        ...
        describe("project with an old version of solidity (multiple compilers)", function () {
            (0, environment_1.useEnvironment)("old-solidity-version-multiple-compilers.js");
            it("should throw an error", async function () {
                await (0, errors_1.expectHardhatErrorAsync)(async () => {
                    await this.env.run("compile");
                }, errors_list_1.ERRORS.BUILTIN_TASKS.COMPILE_TASK_UNSUPPORTED_SOLC_VERSION);
