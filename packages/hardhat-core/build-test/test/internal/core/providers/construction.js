...
describe("Network config typeguards", async () => {
    it("Should recognize HDAccountsConfig", () => {
        chai_1.assert.isTrue((0, construction_1.isHDAccountsConfig)({ mnemonic: "asdads" }));
        chai_1.assert.isFalse((0, construction_1.isHDAccountsConfig)({ initialIndex: 1 }));
...
describe("Base provider creation", () => {
    it("Should create a valid HTTP provider and wrap it", () => {
        const provider = (0, construction_1.createProvider)("net", Object.assign({ url: "http://localhost:8545" }, default_config_1.defaultHttpNetworkParams));
...
describe("Base providers wrapping", () => {
    ...
    describe("Accounts wrapping", () => {
        it("Should wrap with a list of private keys as accounts", async () => {
            const provider = (0, construction_1.applyProviderWrappers)(mockedProvider, {
                accounts: [
    ...
    describe("Sender wrapping", () => {
        beforeEach(async () => {
            ...
            mockedProvider.setReturnValue("eth_gasPrice", (0, base_types_1.numberToRpcQuantity)(12));
        });
    ...
    describe("Gas wrapping", () => {
        ...
        beforeEach(() => {
            mockedProvider.setReturnValue("eth_estimateGas", (0, base_types_1.numberToRpcQuantity)(123));
            mockedProvider.setReturnValue("eth_gasPrice", (0, base_types_1.numberToRpcQuantity)(123));
        });
        ...
        it("Should use the gasMultiplier", async () => {
            ...
            const [tx] = mockedProvider.getLatestParams("eth_sendTransaction");
            chai_1.assert.equal(tx.gas, (0, base_types_1.numberToRpcQuantity)(Math.floor(123 * OTHER_GAS_MULTIPLIER)));
        });
    ...
    describe("Gas price wrapping", () => {
        ...
        it("Should wrap with an auto gas price provider if 'auto' is used", async () => {
            ...
            chai_1.assert.equal(gasPrice, (0, base_types_1.numberToRpcQuantity)(123));
        });
        ...
        it("Should wrap with a fixed gas price provider if a number is used", async () => {
            ...
            chai_1.assert.equal(gasPrice, (0, base_types_1.numberToRpcQuantity)(789));
        });
    });
    describe("Chain ID wrapping", () => {
        it("Should wrap with a chain id validation provider if a chainId is used", async () => {
            ...
            await (0, errors_1.expectHardhatErrorAsync)(() => provider.request({ method: "eth_getAccounts", params: [] }), errors_list_1.ERRORS.NETWORK.INVALID_GLOBAL_CHAIN_ID);
        });
    });
