...
describe("ChainIdValidatorProvider", () => {
    it("should fail when configured chain id dont match the real chain id", async () => {
        ...
        await (0, errors_1.expectHardhatErrorAsync)(() => wrapper.request({ method: "eth_getAccounts", params: [] }), errors_list_1.ERRORS.NETWORK.INVALID_GLOBAL_CHAIN_ID);
    });
});
...
describe("ProviderWrapperWithChainId", function () {
    it("Should call the provider only once", async function () {
        ...
        mockedProvider.setReturnValue("eth_chainId", (0, base_types_1.numberToRpcQuantity)(1));
