...
describe("Compilation jobs", function () {
    describe("createCompilationJobFromFile", function () {
        describe("single file", function () {
            it("valid compiler", async function () {
                ...
                const [dependencyGraph, [Foo]] = await (0, helpers_1.createMockData)([
                    { file: FooMock },
                ]);
                const compilationJobOrError = await (0, compilation_job_1.createCompilationJobFromFile)(dependencyGraph, Foo, solcConfig055);
            ...
            it("newest compiler is used", async function () {
                ...
                const compilationJobOrError = await (0, compilation_job_1.createCompilationJobFromFile)(dependencyGraph, Foo, solcConfig055and066);
                const compilationJob = assertIsJob(compilationJobOrError);
            ...
            it("overriden compiler is used", async function () {
                ...
                const compilationJobOrError = await (0, compilation_job_1.createCompilationJobFromFile)(dependencyGraph, Foo, Object.assign(Object.assign({}, solcConfig066), { overrides: {
            ...
            it("invalid overriden compiler", async function () {
                ...
                const compilationJobOrError = await (0, compilation_job_1.createCompilationJobFromFile)(dependencyGraph, Foo, Object.assign(Object.assign({}, solcConfig055), { overrides: {
        ...
        describe("two files", function () {
            ...
            it("file imported is included in job", async function () {
                const FooMock = new helpers_1.MockFile("Foo", ["^0.5.0"]);
                const BarMock = new helpers_1.MockFile("Bar", ["^0.5.0"]);
                const [dependencyGraph, [Foo, Bar]] = await (0, helpers_1.createMockData)([
        ...
        describe("three files", function () {
            it("transitive dependency", async function () {
                ...
                const [dependencyGraph, [Foo, Bar, Qux]] = await (0, helpers_1.createMockData)([
                    { file: FooMock, dependencies: [BarMock] },
            ...
            it("imported by one and importing one", async function () {
                ...
                const BarMock = new helpers_1.MockFile("Bar", ["^0.5.0"]);
                const QuxMock = new helpers_1.MockFile("Qux", ["^0.5.0"]);
                const [dependencyGraph, [, Bar, Qux]] = await (0, helpers_1.createMockData)([
                    { file: FooMock, dependencies: [BarMock] },
                    { file: BarMock, dependencies: [QuxMock] },
                    { file: QuxMock },
                ]);
                const compilationJobOrError = await (0, compilation_job_1.createCompilationJobFromFile)(dependencyGraph, Bar, solcConfig055);
                const compilationJob = assertIsJob(compilationJobOrError);
                chai_1.assert.equal(compilationJob.getSolcConfig().version, "0.5.5");
    ...
    describe("createCompilationJobsFromConnectedComponent", function () {
        it("single file (success)", async function () {
            const FooMock = new helpers_1.MockFile("Foo", ["^0.5.0"]);
            const [dependencyGraph] = await (0, helpers_1.createMockData)([{ file: FooMock }]);
            const { jobs, errors } = await (0, compilation_job_1.createCompilationJobsFromConnectedComponent)(dependencyGraph, (file) => (0, compilation_job_1.createCompilationJobFromFile)(dependencyGraph, file, solcConfig055));
        ...
        it("files without solc bug", async function () {
            ...
            const [dependencyGraph] = await (0, helpers_1.createMockData)([
        ...
        it("files with solc bug", async function () {
            ...
            const { jobs, errors } = await (0, compilation_job_1.createCompilationJobsFromConnectedComponent)(dependencyGraph, (file) => (0, compilation_job_1.createCompilationJobFromFile)(dependencyGraph, file, solcConfig055Optimized));
            chai_1.assert.lengthOf(jobs, 1);
            chai_1.assert.isEmpty(errors);
        });
    });
