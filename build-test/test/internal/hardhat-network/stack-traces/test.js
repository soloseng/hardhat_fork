...
const return_data_1 = require("../../../../src/internal/hardhat-network/provider/return-data");
...
async function compileIfNecessary(testDir, sources, compilerOptions) {
    ...
    const [compilerInput, compilerOutput] = await (0, compilation_1.compileFiles)(sources, compilerOptions);
...
function compareStackTraces(txIndex, trace, description, runs) {
    chai_1.assert.equal(trace.length, description.length, `Expected a trace of length ${description.length} but got one with length ${trace.length}`);
    for (let i = 0; i < trace.length; i++) {
        ...
        else if (actualMessage instanceof return_data_1.ReturnData &&
        ...
        if (expected.errorCode !== undefined) {
            const actualErrorCode = actual.errorCode;
            chai_1.assert.isDefined(actualErrorCode, `Stack trace of tx ${txIndex} entry ${i} should have an errorCode`);
            const actualErrorCodeHex = actualErrorCode.toString("hex");
            chai_1.assert.isTrue(expected.errorCode === actualErrorCodeHex, `Stack trace of tx ${txIndex} entry ${i} has errorCode ${actualErrorCodeHex} and should have ${expected.errorCode}`);
        }
        else if ("errorCode" in actual) {
            chai_1.assert.isUndefined(actual.errorCode, `Stack trace of tx ${txIndex} entry ${i} shouldn't have errorCode`);
        }
...
async function runTest(testDir, testDefinition, sources, compilerOptions) {
    ...
    const bytecodes = (0, compiler_to_model_1.createModelsAndDecodeBytecodes)(compilerOptions.solidityVersion, compilerInput, compilerOutput);
    ...
    const vm = await (0, execution_1.instantiateVm)();
    ...
    for (const [txIndex, tx] of testDefinition.transactions.entries()) {
        ...
        try {
            ...
            (0, debug_1.printMessageTrace)(decodedTrace);
            throw error;
        }
        if (trace.error !== undefined) {
            ...
            try {
                ...
                if (testDefinition.print !== undefined && testDefinition.print) {
                    ...
                    (0, debug_1.printStackTrace)(stackTrace);
                }
            }
            catch (err) {
                (0, debug_1.printMessageTrace)(decodedTrace);
    ...
    for (const [file, fileLibs] of Object.entries(bytecode.linkReferences)) {
        ...
        for (const [libName, references] of Object.entries(fileLibs)) {
            ...
            for (const ref of references) {
                code = (0, library_utils_1.linkHexStringBytecode)(code, address.address.toString("hex"), ref.start);
            }
        }
    }
...
async function runDeploymentTransactionTest(txIndex, tx, vm, compilerOutput, txIndexToContract) {
    ...
    const params = (0, execution_1.encodeConstructorParams)(contract.abi, tx.params !== undefined ? tx.params : []);
    const data = Buffer.concat([deploymentBytecode, params]);
    const trace = await (0, execution_1.traceTransaction)(vm, {
        value: tx.value,
        data,
        gasLimit: tx.gas,
    });
...
async function runCallTransactionTest(txIndex, tx, vm, compilerOutput, contract) {
    ...
    if (tx.data !== undefined) {
        data = (0, ethereumjs_util_1.toBuffer)(tx.data);
    }
    else if (tx.function !== undefined) {
        data = (0, execution_1.encodeCall)(compilerContract.abi, tx.function, tx.params !== undefined ? tx.params : []);
    }
    else {
        data = Buffer.from([]);
    }
...
const solidity05Compilers = solidityCompilers.filter(({ solidityVersion }) => semver_1.default.satisfies(solidityVersion, "^0.5.0"));
const solidity06Compilers = solidityCompilers.filter(({ solidityVersion }) => semver_1.default.satisfies(solidityVersion, "^0.6.0"));
const solidity07Compilers = solidityCompilers.filter(({ solidityVersion }) => semver_1.default.satisfies(solidityVersion, "^0.7.0"));
const solidity08Compilers = solidityCompilers.filter(({ solidityVersion }) => semver_1.default.satisfies(solidityVersion, "^0.8.0"));
describe("Stack traces", function () {
    ...
    if (customSolcPath !== undefined) {
        ...
        if (!path_1.default.isAbsolute(customSolcPath)) {
    ...
    before("Download solcjs binaries", async function () {
        ...
        for (const p of paths) {
            await (0, compilation_1.downloadSolc)(p);
        }
    });
...
describe("Solidity support", function () {
    it("check that the latest tested version is within the supported version range", async function () {
        const latestSupportedVersion = solidityCompilers
            ...
            .sort(semver_1.default.compare)[solidityCompilers.length - 1];
        chai_1.assert.isTrue(semver_1.default.satisfies(latestSupportedVersion, solidityTracer_1.SUPPORTED_SOLIDITY_VERSION_RANGE), `Expected ${latestSupportedVersion} to be within the ${solidityTracer_1.SUPPORTED_SOLIDITY_VERSION_RANGE} range`);
        const nextPatchVersion = semver_1.default.inc(latestSupportedVersion, "patch");
        const nextMinorVersion = semver_1.default.inc(latestSupportedVersion, "minor");
        const nextMajorVersion = semver_1.default.inc(latestSupportedVersion, "major");
        chai_1.assert.isFalse(semver_1.default.satisfies(nextPatchVersion, solidityTracer_1.SUPPORTED_SOLIDITY_VERSION_RANGE), `Expected ${nextPatchVersion} to not be within the ${solidityTracer_1.SUPPORTED_SOLIDITY_VERSION_RANGE} range`);
        chai_1.assert.isFalse(semver_1.default.satisfies(nextMinorVersion, solidityTracer_1.SUPPORTED_SOLIDITY_VERSION_RANGE), `Expected ${nextMinorVersion} to not be within the ${solidityTracer_1.SUPPORTED_SOLIDITY_VERSION_RANGE} range`);
        chai_1.assert.isFalse(semver_1.default.satisfies(nextMajorVersion, solidityTracer_1.SUPPORTED_SOLIDITY_VERSION_RANGE), `Expected ${nextMajorVersion} to not be within the ${solidityTracer_1.SUPPORTED_SOLIDITY_VERSION_RANGE} range`);
    });
});
