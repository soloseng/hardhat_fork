...
const senderAddress = (0, ethereumjs_util_1.privateToAddress)(senderPrivateKey);
async function instantiateVm() {
    const account = ethereumjs_util_1.Account.fromAccountData({ balance: 1e15 });
    const vm = new vm_1.default({ activatePrecompiles: true });
    await vm.stateManager.putAccount(new ethereumjs_util_1.Address(senderAddress), account);
    return vm;
}
...
async function traceTransaction(vm, txData) {
    var _a;
    const tx = new tx_1.Transaction(Object.assign(Object.assign({ value: 0, gasPrice: 1, nonce: await getNextPendingNonce(vm) }, txData), { 
...
async function getNextPendingNonce(vm) {
    const acc = await vm.stateManager.getAccount(new ethereumjs_util_1.Address(senderAddress));
