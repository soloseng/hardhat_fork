    ...
    describe("Using fake providers", function () {
        describe("Caching", () => {
            ...
            function getStorageAt(blockNumber) {
                return clientWithFakeProvider.getStorageAt(constants_1.DAI_ADDRESS, new ethereumjs_util_1.BN(constants_1.DAI_TOTAL_SUPPLY_STORAGE_POSITION), new ethereumjs_util_1.BN(blockNumber));
            }
        ...
        describe("Retry on Infura's error", () => {
            ...
            it("makes a retry on the 'header not found' error", async () => {
                ...
                const value = await clientWithFakeProvider.getStorageAt(constants_1.DAI_ADDRESS, new ethereumjs_util_1.BN(constants_1.DAI_TOTAL_SUPPLY_STORAGE_POSITION), new ethereumjs_util_1.BN(120));
                chai_1.assert.equal(fakeProvider.request.callCount, 2);
                chai_1.assert.isTrue(value.equals((0, ethereumjs_util_1.toBuffer)(response)));
            });
            it("does not retry more than once", async () => {
                ...
                const clientWithFakeProvider = new client_1.JsonRpcClient(fakeProvider, 1, 123, 3);
                await chai_1.assert.isRejected(clientWithFakeProvider.getStorageAt(constants_1.DAI_ADDRESS, new ethereumjs_util_1.BN(constants_1.DAI_TOTAL_SUPPLY_STORAGE_POSITION), new ethereumjs_util_1.BN(120)), "header not found");
            });
            it("does not retry on a different error", async () => {
                ...
                const clientWithFakeProvider = new client_1.JsonRpcClient(fakeProvider, 1, 123, 3);
                await chai_1.assert.isRejected(clientWithFakeProvider.getStorageAt(constants_1.DAI_ADDRESS, new ethereumjs_util_1.BN(constants_1.DAI_TOTAL_SUPPLY_STORAGE_POSITION), new ethereumjs_util_1.BN(120)), "different error");
            });
    ...
    describe("Using actual providers", function () {
        providers_1.FORKED_PROVIDERS.forEach(({ rpcProvider, jsonRpcUrl }) => {
            ...
            describe(`Using ${rpcProvider}`, () => {
                ...
                let forkNumber;
                beforeEach(async () => {
                    const clientResult = await (0, makeForkClient_1.makeForkClient)({ jsonRpcUrl });
                ...
                describe("Basic tests", () => {
                    ...
                    it("can actually fetch real json-rpc", async () => {
                        ...
                        const tx = await client.getTransactionByHash((0, ethereumjs_util_1.toBuffer)("0xc008e9f9bb92057dd0035496fbf4fb54f66b4b18b370928e46d6603933054d5a"));
                ...
                describe("eth_getBlockByHash", () => {
                    ...
                    it("returns null for non-existent block", async () => {
                        const block = await client.getBlockByHash((0, random_1.randomHashBuffer)(), true);
                        chai_1.assert.isNull(block);
                    });
                });
                describe("eth_getStorageAt", () => {
                    it("can fetch value from storage of an existing contract", async () => {
                        const totalSupply = await client.getStorageAt(constants_1.DAI_ADDRESS, new ethereumjs_util_1.BN(constants_1.DAI_TOTAL_SUPPLY_STORAGE_POSITION), forkNumber);
                    ...
                    it("can fetch empty value from storage of an existing contract", async () => {
                        const value = await client.getStorageAt(constants_1.DAI_ADDRESS, new ethereumjs_util_1.BN("baddcafe", 16), forkNumber);
                    ...
                    it("can fetch empty value from storage of a non-existent contract", async () => {
                        const value = await client.getStorageAt(constants_1.EMPTY_ACCOUNT_ADDRESS, new ethereumjs_util_1.BN(1), forkNumber);
                        const valueBN = new ethereumjs_util_1.BN(value);
                        chai_1.assert.isTrue(valueBN.eqn(0));
                    });
                });
                describe("getTransactionByHash", () => {
                    ...
                    it("returns null for non-existent transactions", async () => {
                        const transaction = await client.getTransactionByHash((0, random_1.randomHashBuffer)());
                ...
                describe("getTransactionReceipt", () => {
                    ...
                    it("returns null for non-existent transactions", async () => {
                        const transaction = await client.getTransactionReceipt((0, random_1.randomHashBuffer)());
                        chai_1.assert.equal(transaction, null);
                    });
                });
                describe("getLogs", () => {
                    it("can fetch existing logs", async () => {
                        const logs = await client.getLogs({
                            ...
                            address: (0, ethereumjs_util_1.toBuffer)("0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2"),
                        });
