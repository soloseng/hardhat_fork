...
describe("ForkBlockchain", () => {
    ...
    let common;
    let fb;
    function createBlock(parent, difficulty = (0, ethereumjs_util_1.zeros)(32)) {
        return block_1.Block.fromBlockData({
            header: {
                number: new ethereumjs_util_1.BN(parent.header.number).addn(1),
                parentHash: parent.hash(),
                difficulty: new ethereumjs_util_1.BN(difficulty),
            },
        }, { common });
    }
    ...
    describe("getBlock", () => {
        ...
        it("returns undefined for non-existent block", async () => {
            chai_1.assert.equal(await fb.getBlock((0, random_1.randomHashBuffer)()), undefined);
        ...
        it("can get remote block object with create transaction", async () => {
            ...
            chai_1.assert.isTrue((block === null || block === void 0 ? void 0 : block.transactions[daiCreateTxPosition].to) === undefined);
            chai_1.assert.isTrue(block === null || block === void 0 ? void 0 : block.transactions[daiCreateTxPosition].hash().equals((0, ethereumjs_util_1.toBuffer)("0xb95343413e459a0f97461812111254163ae53467855c0d73e0f1e7c5b8442fa3")));
        });
    ...
    describe("putBlock", () => {
        ...
        it("rejects blocks with invalid block number", async () => {
            await chai_1.assert.isRejected(fb.addBlock(block_1.Block.fromBlockData({ header: { number: forkBlockNumber.addn(2) } })), Error, "Invalid block number");
        });
        it("rejects blocks with invalid parent hash after the first block in the forked blockchain", async () => {
            const block = createBlock(await fb.getLatestBlock());
            await fb.addBlock(block);
            await chai_1.assert.isRejected(fb.addBlock(block_1.Block.fromBlockData({ header: { number: forkBlockNumber.addn(2) } })), Error, "Invalid parent hash");
        });
    ...
    describe("delBlock", () => {
        ...
        it("is possible to add a block after delete", async () => {
            const block = createBlock(await fb.getLatestBlock());
            const otherBlock = createBlock(await fb.getLatestBlock(), (0, random_1.randomHashBuffer)());
            await fb.addBlock(block);
        ...
        it("throws when hash of non-existent block is given", async () => {
            chai_1.assert.throws(() => fb.deleteBlock(block_1.Block.fromBlockData().hash()), Error, "Block not found");
        });
    ...
    describe("deleteAllFollowingBlocks", () => {
        ...
        it("throws if given block is not present in blockchain", async () => {
            ...
            const fakeBlockOne = createBlock(await fb.getLatestBlock(), (0, random_1.randomHashBuffer)());
            await fb.addBlock(blockOne);
    ...
    describe("getTransactionReceipt", () => {
        ...
        it("returns remote receipts", async () => {
            ...
            chai_1.assert.equal(receipt === null || receipt === void 0 ? void 0 : receipt.transactionHash, (0, ethereumjs_util_1.bufferToHex)(constants_1.FIRST_TX_HASH_OF_10496585));
        });
    ...
    describe("getLogs", () => {
        it("works like BlockchainData.getLogs for new blocks", async () => {
            ...
            const log1 = (0, blockchain_1.createTestLog)(number);
            const log2 = (0, blockchain_1.createTestLog)(number);
            ...
            const log3 = (0, blockchain_1.createTestLog)(number);
            ...
            const log4 = (0, blockchain_1.createTestLog)(number.addn(1));
        ...
        it("supports remote blocks", async () => {
            ...
            const logs = await fb.getLogs({
                ...
                addresses: [(0, ethereumjs_util_1.toBuffer)("0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2")],
                normalizedTopics: [],
            });
            chai_1.assert.equal(logs.length, 12);
            chai_1.assert.deepEqual(logs.map((l) => l.logIndex), [
                "0x1",
                "0x4",
                "0xd",
                "0xe",
