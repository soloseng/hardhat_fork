...
const useProvider_1 = require("../../../helpers/useProvider");
describe("Eth module - hardfork dependant tests", function () {
    function useProviderAndCommon(hardfork) {
        (0, useProvider_1.useProvider)({ hardfork });
    ...
    const privateKey = Buffer.from(providers_1.DEFAULT_ACCOUNTS[1].privateKey.slice(2), "hex");
    function getSampleSignedTx(common) {
        const tx = tx_1.Transaction.fromTxData({
    ...
    function getSampleSignedAccessListTx(common) {
        const tx = tx_1.AccessListEIP2930Transaction.fromTxData({
            to: "0x1111111111111111111111111111111111111111",
            gasLimit: 21000,
            gasPrice: 10,
        }, {
    ...
    function getSampleSignedEIP1559Tx(common) {
    ...
    function getEffectiveGasPrice(baseFee, maxFeePerGas, maxPriorityFeePerGas) {
        return ethereumjs_util_1.BN.min(maxFeePerGas.sub(baseFee), maxPriorityFeePerGas).add(baseFee);
    }
        ...
        describe("chain id validation", function () {
            describe("In a hardfork without access list but with EIP-155", function () {
                ...
                it("Should validate the chain id if sent to eth_sendTransaction", async function () {
                    ...
                    await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_sendTransaction", [{ from: sender, to: sender, chainId: (0, base_types_1.numberToRpcQuantity)(1) }], "Invalid chainId");
                });
                it("Should validate the chain id if an EIP-155 tx is sent with eth_sendRawTransaction", async function () {
                    const signedTx = getSampleSignedTx(new common_1.default({ chain: "mainnet", hardfork: "spuriousDragon" }));
                    const serialized = (0, base_types_1.bufferToRpcData)(signedTx.serialize());
                    await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_sendRawTransaction", [serialized], "signed for another chain");
                });
            });
            describe("In a hardfork with access list", function () {
                ...
                it("Should validate the chain id if sent to eth_sendTransaction using access list", async function () {
                    const [sender] = await this.provider.send("eth_accounts");
                    await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_sendTransaction", [
                        {
                            from: sender,
                            to: sender,
                            chainId: (0, base_types_1.numberToRpcQuantity)(1),
                            accessList: [],
                        },
                    ], "Invalid chainId");
                });
                it("Should validate the chain id in eth_sendRawTransaction using an access list tx", async function () {
                    const signedTx = getSampleSignedAccessListTx(new common_1.default({ chain: "mainnet", hardfork: "berlin" }));
                    const serialized = (0, base_types_1.bufferToRpcData)(signedTx.serialize());
                    await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_sendRawTransaction", [serialized], "Trying to send a raw transaction with an invalid chainId");
                });
            });
            describe("In a hardfork with EIP-1559", function () {
                for (const hardfork of ["london", "arrowGlacier"]) {
                    ...
                    it(`Should validate the chain id in eth_sendRawTransaction using an eip-1559 tx when ${hardfork} is activated`, async function () {
                        const signedTx = getSampleSignedEIP1559Tx(new common_1.default({ chain: "mainnet", hardfork }));
                        const serialized = (0, base_types_1.bufferToRpcData)(signedTx.serialize());
                        await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_sendRawTransaction", [serialized], "Trying to send a raw transaction with an invalid chainId");
                    });
                }
        ...
        describe("Transaction type validation by hardfork", function () {
            function rejectsSendTransactionWithAccessList() {
                it("Should reject an eth_sendTransaction if an access list was provided", async function () {
                    const [sender] = await this.provider.send("eth_accounts");
                    await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_sendTransaction", [{ from: sender, to: sender, accessList: [] }], "Access list received but is not supported by the current hardfork");
                });
            }
            ...
            function rejectsSendRawTransactionWithAccessListTx() {
                it("Should reject an eth_sendRawTransaction if the tx uses an access list", async function () {
                    ...
                    const serialized = (0, base_types_1.bufferToRpcData)(signedTx.serialize());
                    await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_sendRawTransaction", [serialized], "Trying to send an EIP-2930 transaction");
                });
            }
            function rejectsSendRawTransactionWithEIP1559Tx() {
                for (const hardfork of ["london", "arrowGlacier"]) {
                    it(`Should reject an eth_sendRawTransaction if the tx uses an EIP-1559 tx when ${hardfork} is activated`, async function () {
                        ...
                        const serialized = (0, base_types_1.bufferToRpcData)(signedTx.serialize());
                        await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_sendRawTransaction", [serialized], "Trying to send an EIP-1559 transaction");
                    });
                }
            }
            describe("Without EIP155 nor access list", function () {
                ...
                it("Should reject an eth_sendRawTransaction if signed with EIP-155", async function () {
                    ...
                    const serialized = (0, base_types_1.bufferToRpcData)(signedTx.serialize());
                    await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_sendRawTransaction", [serialized], "Trying to send an EIP-155 transaction");
                });
        ...
        describe("Call and estimate gas types validation by hardfork", function () {
            describe("Running a hardfork without access list", function () {
                ...
                it("Should reject an eth_call if an access list was provided", async function () {
                    const [sender] = await this.provider.send("eth_accounts");
                    await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_call", [{ from: sender, to: sender, accessList: [] }], "Access list received but is not supported by the current hardfork");
                });
                it("Should reject an eth_call with EIP-1559 fields", async function () {
                    const [sender] = await this.provider.send("eth_accounts");
                    await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_call", [{ from: sender, to: sender, maxFeePerGas: "0x1" }], "EIP-1559 style fee params (maxFeePerGas or maxPriorityFeePerGas) received but they are not supported by the current hardfork");
                });
                it("Should reject an eth_estimateGas if an access list was provided", async function () {
                    const [sender] = await this.provider.send("eth_accounts");
                    await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_estimateGas", [
                ...
                it("Should reject an eth_estimateGas with EIP-1559 fields", async function () {
                    const [sender] = await this.provider.send("eth_accounts");
                    await (0, assertions_1.assertInvalidArgumentsError)(this.provider, "eth_estimateGas", [
                        { from: sender, to: sender, maxFeePerGas: (0, base_types_1.numberToRpcQuantity)(1) },
                    ]);
                });
            });
            describe("Running a hardfork without access list", function () {
    ...
    describe("Block formatting", function () {
        describe("When running EIP-1559", function () {
            for (const hardfork of ["london", "arrowGlacier"]) {
                ...
                it(`Should have a baseFeePerGas field when ${hardfork} is activated`, async function () {
                    ...
                    chai_1.assert.isDefined(block.baseFeePerGas);
                });
            }
        });
        describe("When not running EIP-1559", function () {
            ...
            it("Should not have a baseFeePerGas field", async function () {
                const block = await this.provider.send("eth_getBlockByNumber", ["latest", false]);
                chai_1.assert.isUndefined(block.baseFeePerGas);
            });
        });
    });
        ...
        describe("Transactions formatting", function () {
            describe("Before berlin", function () {
                ...
                it("Should not include the fields type, chainId and accessList", async function () {
                    ...
                    chai_1.assert.isUndefined(tx.type);
                    chai_1.assert.isUndefined(tx.chainId);
                    chai_1.assert.isUndefined(tx.accessList);
                });
            });
            describe("After berlin", function () {
                ...
                describe("legacy tx", function () {
                    it("Should include the field type, but not chainId and accessList", async function () {
                        ...
                        chai_1.assert.equal(tx.type, (0, base_types_1.numberToRpcQuantity)(0));
                        chai_1.assert.isUndefined(tx.chainId);
                        chai_1.assert.isUndefined(tx.accessList);
                    });
                });
                describe("access list tx", function () {
                    it("Should include the fields type,chainId and accessList", async function () {
                        ...
                        chai_1.assert.equal(tx.type, (0, base_types_1.numberToRpcQuantity)(1));
                        chai_1.assert.equal(tx.chainId, (0, base_types_1.numberToRpcQuantity)(this.common.chainId()));
                        chai_1.assert.deepEqual(tx.accessList, accessList);
                    });
                    it("Should accept access lists with null storageKeys", async function () {
                        ...
                        chai_1.assert.equal(tx.type, (0, base_types_1.numberToRpcQuantity)(1));
                        chai_1.assert.equal(tx.chainId, (0, base_types_1.numberToRpcQuantity)(this.common.chainId()));
                        chai_1.assert.deepEqual(tx.accessList, [
                            {
                                address: "0x1234567890123456789012345678901234567890",
                                storageKeys: [],
                            },
                ...
                for (const hardfork of ["london", "arrowGlacier"]) {
                    ...
                    describe(`EIP-1559 txs when ${hardfork} is activated`, function () {
                        it("Should include gasPrice, maxBaseFeePerGas and maxPriorityFeePerGas for EIP-1559", async function () {
                            ...
                            const txHash = await this.provider.send("eth_sendRawTransaction", [serialized]);
                            const pendingRpcTx = await this.provider.send("eth_getTransactionByHash", [txHash]);
                            chai_1.assert.equal(pendingRpcTx.maxFeePerGas, (0, base_types_1.numberToRpcQuantity)(signedTx.maxFeePerGas));
                            chai_1.assert.equal(pendingRpcTx.maxPriorityFeePerGas, (0, base_types_1.numberToRpcQuantity)(signedTx.maxPriorityFeePerGas));
                            chai_1.assert.equal(pendingRpcTx.gasPrice, (0, base_types_1.numberToRpcQuantity)(signedTx.maxFeePerGas));
                            ...
                            const block = await this.provider.send("eth_getBlockByNumber", ["latest", false]);
                            const minedTx = await this.provider.send("eth_getTransactionByHash", [txHash]);
                            const effectiveGasPrice = getEffectiveGasPrice((0, base_types_1.rpcQuantityToBN)(block.baseFeePerGas), signedTx.maxFeePerGas, signedTx.maxPriorityFeePerGas);
                            chai_1.assert.equal(minedTx.gasPrice, (0, base_types_1.numberToRpcQuantity)(effectiveGasPrice));
                        });
                    });
                }
            });
        ...
        describe("Receipts formatting", function () {
            describe("Before byzantium", function () {
                ...
                it("Should have a root field, and shouldn't have a status one nor type", async function () {
                    ...
                    chai_1.assert.isDefined(receipt.root);
                    chai_1.assert.isUndefined(receipt.status);
                    chai_1.assert.isUndefined(receipt.type);
                });
            });
            describe("After byzantium, before berlin", function () {
                ...
                it("Should have a status field and not a root one nor type", async function () {
                    ...
                    const receipt = await this.provider.send("eth_getTransactionReceipt", [tx]);
                    chai_1.assert.isDefined(receipt.status);
                    chai_1.assert.isUndefined(receipt.root);
                    chai_1.assert.isUndefined(receipt.type);
                });
            });
            describe("After berlin, before london", function () {
                ...
                it("Should have status and type fields and not a root one", async function () {
                    ...
                    chai_1.assert.isDefined(receipt.status);
                    chai_1.assert.isUndefined(receipt.root);
                    chai_1.assert.equal(receipt.type, "0x0");
                });
                it("Should not have an effectiveGasPrice field", async function () {
                    ...
                    const receipt = await this.provider.send("eth_getTransactionReceipt", [tx]);
                    chai_1.assert.isUndefined(receipt.effectiveGasPrice);
                });
            });
            describe("After london", function () {
                for (const hardfork of ["london", "arrowGlacier"]) {
                        ...
                        const tx = await this.provider.send("eth_sendTransaction", [
                            {
                                ...
                                maxPriorityFeePerGas: (0, base_types_1.numberToRpcQuantity)(maxPriorityPerGas),
                            },
                        ]);
                        const receipt = await this.provider.send("eth_getTransactionReceipt", [tx]);
                        const block = await this.provider.send("eth_getBlockByNumber", ["latest", false]);
                        const baseFee = (0, base_types_1.rpcQuantityToBN)(block.baseFeePerGas);
                        const effectiveGasPrice = getEffectiveGasPrice(baseFee, maxFeePerGas, maxPriorityPerGas);
                        chai_1.assert.equal(receipt.type, "0x2");
                        chai_1.assert.equal(receipt.effectiveGasPrice, (0, base_types_1.numberToRpcQuantity)(effectiveGasPrice));
                    });
                    it(`should have an effectiveGasPrice field for Access List txs when ${hardfork} is activated`, async function () {
                        ...
                        const block = await this.provider.send("eth_getBlockByNumber", ["latest", false]);
                        const baseFee = (0, base_types_1.rpcQuantityToBN)(block.baseFeePerGas);
                        const effectiveGasPrice = getEffectiveGasPrice(baseFee, gasPrice, gasPrice);
                        chai_1.assert.equal(receipt.type, "0x1");
                        chai_1.assert.equal(receipt.effectiveGasPrice, (0, base_types_1.numberToRpcQuantity)(effectiveGasPrice));
                    });
    ...
    describe("Impersonated accounts", function () {
        describe("Berlin hardfork", function () {
            ...
            it("should allow sending access list txs from impersonated accounts", async function () {
                ...
                await this.provider.send("eth_sendTransaction", [
                    {
                        ...
                        to: impersonated,
                        value: (0, base_types_1.numberToRpcQuantity)(new ethereumjs_util_1.BN("100000000000000000")),
                    },
                ]);
                ...
                chai_1.assert.isDefined(tx.accessList);
                chai_1.assert.isArray(tx.accessList);
            });
        });
    ...
    describe("Access list transactions", function () {
        ...
        const WRITE_STORAGE_KEYS = [
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(0), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(1), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(2), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(3), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(4), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(5), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(6), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(7), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(8), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(9), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(10), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(11), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(12), 32),
            (0, base_types_1.bufferToRpcData)((0, ethereumjs_util_1.toBuffer)(13), 32),
        ];
        function abiEncodeUint(uint) {
            return new ethereumjs_util_1.BN(uint).toBuffer("be", 32).toString("hex");
        }
        let contract;
        let txData;
        beforeEach(async function () {
            contract = await (0, transactions_1.deployContract)(this.provider, TEST_CONTRACT_DEPLOYMENT_BYTECODE);
            txData = {
                to: contract,
                data: TEST_FUNCTION_SELECTOR +
        ...
        describe("Validate access list test contract", function () {
            it("Should revert if the max gas is forwarded", async function () {
                await chai_1.assert.isRejected(this.provider.send("eth_call", [
                    Object.assign(Object.assign({}, txData), { accessList: undefined }),
                ...
                await this.provider.send("eth_call", [
                    {
                        to: contract,
                        data: TEST_FUNCTION_SELECTOR +
                            abiEncodeUint(MAX_GAS_TO_FORWARD_THAT_FAILS_WITHOUT_ACCESS_LIST + 1),
                    },
                ]);
            });
        });
        ...
        describe("eth_sendRawTransaction", function () {
            it("should use the access list if an EIP-2930 tx is sent", async function () {
                const unsignedTx = tx_1.AccessListEIP2930Transaction.fromTxData(Object.assign(Object.assign({}, txData), { gasPrice: 10, gasLimit: 1000000 }), {
                    common: this.common,
                });
                const signedTx = unsignedTx.sign(privateKey);
                const txHash = await this.provider.send("eth_sendRawTransaction", [
                    (0, base_types_1.bufferToRpcData)(signedTx.serialize()),
                ]);
                const tx = await this.provider.send("eth_getTransactionByHash", [
                    txHash,
                ]);
        ...
        describe("eth_sendTransaction", function () {
            describe("When automining", function () {
                it("Should use an EIP-2930 tx if an access list is sent using a local account", async function () {
                    ...
                    const txHash = await this.provider.send("eth_sendTransaction", [
                        Object.assign(Object.assign({}, txData), { from }),
                    ]);
                    const tx = await this.provider.send("eth_getTransactionByHash", [
                        txHash,
                    ]);
                ...
                it("Should use an EIP-2930 tx if an access list is sent using an impersonated account", async function () {
                    ...
                    const txHash = await this.provider.send("eth_sendTransaction", [
                        Object.assign(Object.assign({}, txData), { from, gasPrice: (0, base_types_1.numberToRpcQuantity)(10) }),
                    ]);
                    const tx = await this.provider.send("eth_getTransactionByHash", [
                        txHash,
                    ]);
            ...
            describe("When txs go through the mempool", function () {
                ...
                it("Should use an EIP-2930 tx if an access list is sent using a local account", async function () {
                    ...
                    const pendingTx = await this.provider.send("eth_getTransactionByHash", [txHash]);
                    chai_1.assert.equal(pendingTx.type, (0, base_types_1.numberToRpcQuantity)(1));
                    await this.provider.send("evm_mine", []);
                    const minedTx = await this.provider.send("eth_getTransactionByHash", [
                        txHash,
                    ]);
                    chai_1.assert.equal(minedTx.type, (0, base_types_1.numberToRpcQuantity)(1));
                });
                it("Should use an EIP-2930 tx if an access list is sent using an impersonated account", async function () {
                    const from = "0x1234567890123456789012345678901234567890";
                    await this.provider.send("hardhat_impersonateAccount", [from]);
    ...
    describe("eth_feeHistory", function () {
        describe("In a hardfork without EIP-1559", function () {
            ...
            it("Should be disabled", async function () {
                await (0, assertions_1.assertInvalidInputError)(this.provider, "eth_feeHistory", ["0x1", "latest"], "eth_feeHistory is disabled. It only works with the London hardfork or a later one.");
            });
        });
        describe("In a hardfork with EIP-1559", function () {
            for (const hardfork of ["london", "arrowGlacier"]) {
