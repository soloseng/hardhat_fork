...
async function expectHardhatErrorAsync(f, errorDescriptor, errorMessage) {
    ...
    try {
        ...
        chai_1.assert.instanceOf(err, errors_1.HardhatError);
        chai_1.assert.equal(err.number, errorDescriptor.number);
        chai_1.assert.notInclude(err.message, "%s", "HardhatError has old-style format tag");
        chai_1.assert.notMatch(err.message, /%[a-zA-Z][a-zA-Z0-9]*%/, "HardhatError has an non-replaced variable tag");
        if (errorMessage !== undefined) {
            if (typeof errorMessage === "string") {
